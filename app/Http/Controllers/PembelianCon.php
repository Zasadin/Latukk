...
class PembelianCon extends Controller
{
    ...
    {
        $kode_pembelian = $request->kodeproduk . date('/d/Y');
        // insert data ke table tbpembelian
        DB::table('pembelian')->insert([
            'kode_produk' => $request->kodeproduk,
            'banyak' => $request->banyak,
            'bayar' => $request->banyak * $request->harga,
            'kode_pembeli' => Auth::user()->id,
            'status' => 'verifikasi'
        ]);
        DB::table('produk')->where('kode', $request->kodeproduk)->update([
            'stok' => $request->stok - $request->banyak,
        ]);
